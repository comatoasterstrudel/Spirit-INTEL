...
class OverworldState extends FlxState
{
			...
			switch (entity.name)
			{
				...
				case "door":
					var door = new Door(entity.values.doorName, entity.values.tag, player, Std.int(entity.x * Constants.overworldPixelScale),
						...
						entity.values.lockedDialogue, entity.values.scriptFunction, entity.values.facingDownwards);
					props.add(door);
	...
	public static function isMusicSameAsLastMusic(roomData:RoomData):Bool
	{
		...
		if (roomData.music != "" && Assets.exists(path))
		{
			if(lastMusic == path) return true;
		}

		return false;
	}

	public static function updateLastMusic():Void{
